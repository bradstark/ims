...
class workflow_uk extends CI_Controller {
...
public function assignweldbay(){
    
    
    $this->load->view('asignweldbay'); 
    
}
public function reassignwkorder(){
    
    
    $this->load->view('reaasign_uk'); 
...
echo "<script type='text/javascript'>alert('$message');</script>";
        $this->load->view('resumeonrepair_uk',$data);
        }
        else{
    $wokid = $this->input->post('wid');
...
$this->work_model_uk->unassignrepaired($wokid);
...
public function assignweldbaywkflow() {  
         if(count($_POST['checkbox'])<>1 ){
               $message = " Please select only one workorder to assign weld bay!";
echo "<script type='text/javascript'>alert('$message');</script>";
         $this->load->view('asignweldbay'); }
         else{
  $weldslot=$this->work_model_uk->get_weld_slotstatus();
  $i=0;
  $repairid=$_POST['checkbox'][$i];
    if ($weldslot->status==0) {
       $row= $this->work_model_uk->get_allonrepair_by_wkid($repairid);
    $data=array('workorderid'=>$row->workorderid,
    ...
    $this->work_model_uk->assignslotandwk(9,$row->equipmentid,$row->workorderid,1,$row->containersize,$row->manufacturer);
    $this->work_model_uk->unassignonrepair($data);
    $this->load->view('viewworkorder_uk');
    }
    else{
         $message = " Weld Bay is already occupuied !";
         echo "<script type='text/javascript'>alert('$message');</script>";
         $this->load->view('asignweldbay'); 
...
public function unassignweldbay()
  {
   ...
   $row= $this->work_model->geteqid($wkid);
       ...
       $size=$row->containersize;
       $man=$row->manufacturer;
      $data=array('workorderid'=>$wkid,
        'equipmentid'=>$equid,
        'containersize'=>$size,
        'manufacturer'=>$man,
                   );
   $this->work_model_uk->assignrepaired($data);     
   $this->work_model_uk->assignslotandwk(9,0,0,0,0,0);
   $this->load->view('reaasign_uk');
}
