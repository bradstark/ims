...
class work_model_uk extends CI_Model {
...
$this->db->insert('workflowdetails'); 
    ...
    public function get_allwkflow() {
     ...
     public function get_allrepaired() {
        $isocontainer = $this->db->order_by('workorderid')
                ->get('repaired')
                ->result_array();
        return $isocontainer;
      ...
      public function get_allonrepair_by_wkid($wkid) {
        $row= $this->db->get_where('onrepair', array('workorderid' => $wkid))->row();
        return $row;
    ...
    public function slotsavailable()
  {
    ...
    $query = $this->db->get_where('slotstatus', array('status' => $key , 'slot_type' => $key1 ));
    if ($query->num_rows() > 0) {
        foreach ($query->result() as $row) {
            $data[] = $row;
        }
      ...
      public function slotsfilled()
  {
    ...
    public function get_by_colorstatus($wid) {
     ...
     public function get_weld_slotstatus() {
        $row= $this->db->get_where('slotstatus', array('slotnumber' => 9))->row();
        return $row;
    }
...
$this->db->update('colorstatus', $data);
 ...
 public function unassignonrepair($data)
  {
      
   $this->db->delete('onrepair', array('workorderid' => $data['workorderid']));
}
public function unassignrepaired($wkid)
  {
      
   $this->db->delete('repaired', array('workorderid' => $wkid));
}
...
public function assignrepaired($onRepair)
{
    
       
   $this->db->set($onRepair);
     
$this->db->insert('repaired'); 
